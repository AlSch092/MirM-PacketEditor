...
public:
	...
	PacketWriter(uint32_t h) : m_pos(0), m_buffer(new unsigned char[bufferLen]), m_length(bufferLen) { Write(h); }
...
void PacketWriter::WriteWideString(const std::wstring &str, size_t len) {
	...
	wcscpy_s((wchar_t *)GetBuffer(m_pos, len * 2), slen + 1, str.c_str()); //CHECK to make sure _s works (second param)
